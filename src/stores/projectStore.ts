  ...
  persist(
    immer((set, get) => ({
      ...
      initializeProject: (projectId: string, userId: string, projectName?: string) => set((state) => {
        // Nettoyer toutes les données si on change de projet pour éviter la contamination
        if (state.projectId !== projectId) {
          console.log('🧹 Nettoyage store pour nouveau projet:', projectId)
          ...
          state.scriptData = {
            ...
            title: projectName ? `Script for project "${projectName}"` : 'Script Sans Titre'
          }
          state.charactersData = []
          state.backgroundsData = []
          state.decorsData = []
          state.scenesData = []
          state.assemblyData = initialState.assemblyData
          state.hasUnsavedChanges = false
          state.lastSavedToDb = null
          state.lastSavedToLocal = new Date()
        } else {
          // Même projet, juste mettre à jour les IDs
          state.projectId = projectId
          state.userId = userId

          // Update script title with project name
          if (projectName && state.scriptData.title === 'Untitled Script') {
            state.scriptData.title = `Script for project "${projectName}"`
          }
        }
      })
    })),
