...
interface ScriptEditorPanelProps {
  ...
  pagesSidebarVisible?: boolean // New prop to detect if pages sidebar is visible
}
...
export default function ScriptEditorPanel({ projectId, pagesSidebarVisible = false }: ScriptEditorPanelProps) {
  ...
  // ✅ CORRECTION: Simplified statistics calculation only (without fileTree)
  const calculateStats = useCallback((content: string): ScriptStats => {
    // Optimization: avoid calculations if content is empty
    if (!content.trim()) {
      return {
        pages: 0,
        panels: 0,
    ...
    // Optimization: use single loop with pre-compiled regex
    const pageRegex = /^PAGE\s+\d+\s*:/
    const chapterRegex = /^(CHAPITRE|CHAPTER)\s+\d+\s*:/
    ...
    // Optimization: more efficient word calculation
  ...
  const updateDisplayContent = useCallback(
    debounce((content: string) => {
      ...
      // ✅ CORRECTION: Generate fileTree with global unique IDs
      const lines = content.split('\n')
      ...
      // Global counters to ensure ID uniqueness
      ...
      lines.forEach((line, index) => {
        ...
        if (pageRegex.test(trimmed)) {
          ...
          currentChapter = {
            ...
            type: 'chapter',
            title: `Chapter ${globalChapterCount}`,
            content: trimmed,
            children: [],
            expanded: true,
            lineNumber: index + 1
  ...
  // Auto-save with authentication verification
  ...
  // Charger les données existantes du script avec isolation correcte
  const loadScriptData = useCallback(async () => {
    if (!user || !projectId) return

    try {
      console.log('🔄 Chargement script pour projet:', projectId, 'utilisateur:', user.id)
      ...
      if (data && data.script_data) {
        console.log('✅ Script trouvé, chargement des données')
        const scriptData = data.script_data

        // Update store with loaded data
        updateScriptData({
          content: scriptData?.script?.content || '',
          title: data.title || 'Untitled Script',
          stats: scriptData?.script?.stats || { pages: 0, panels: 0, chapters: 0, words: 0, characters: 0, dialogues: 0 },
          fileTree: scriptData?.script?.fileTree || []
        })

        // Update local display
        const content = scriptData?.script?.content || ''
        setDisplayContent(content)
        currentContentRef.current = content
        handleContentChange(content)
      } else {
        console.log('📝 New project - empty script')
        // New project - initialize with empty script
        updateScriptData({
          content: '',
  ...
  }, [user, projectId, supabase, handleContentChange, updateScriptData])

  // Nettoyer les données quand on change de projet pour éviter la contamination
  useEffect(() => {
    console.log('🧹 Nettoyage script pour nouveau projet:', projectId)

    // Use updateScriptData to clean all script data
    updateScriptData({
      content: '',
      title: 'Untitled Script',
      stats: { pages: 0, panels: 0, chapters: 0, words: 0, characters: 0, dialogues: 0 },
    ...
    // Also clean local display state
    setDisplayContent('')
    currentContentRef.current = ''

    console.log('✅ Nettoyage terminé pour projet:', projectId)
  }, [projectId, updateScriptData])

  // Charger les données au montage du composant avec isolation correcte
  useEffect(() => {
    if (user && !authLoading && projectId) {
      console.log('🚀 Initialisation script pour projet:', projectId)
      loadScriptData()
    }
  }, [user, authLoading, projectId, loadScriptData])
  ...
  // Simple scroll handler for script structure
  const handleStructureScroll = useCallback((e: React.UIEvent<HTMLDivElement>) => {
    // Prevent propagation for isolation
    e.stopPropagation()
  }, [])
  ...
  return (
    <div className={`h-screen flex flex-col bg-gray-900 text-gray-100 overflow-hidden transition-all duration-300 ${isResizing ? 'select-none' : ''}`} suppressHydrationWarning={true}>
      ...
      <style jsx global>{`
        ...
        /* Simple scrollbar for script structure */
      ...
      {/* Modern and elegant toolbar - Responsive with conditional scrollbar */}
      <div className="bg-gradient-to-r from-gray-800 to-gray-750 border-b border-gray-600 px-2 md:px-4 py-2 flex-shrink-0 shadow-lg" suppressHydrationWarning={true}>
        <div className="flex items-center justify-between" suppressHydrationWarning={true}>
          {/* Insertion buttons in chronological order - Responsive with conditional scrollbar */}
          ...
          {/* Export controls with compact professional design - Responsive */}
      ...
      <div className="flex-1 flex flex-col lg:flex-row min-h-0 overflow-hidden" suppressHydrationWarning={true}>
        ...
        <div className="flex-1 flex flex-col min-h-0 relative" suppressHydrationWarning={true}>
          <div className="p-2 md:p-6" suppressHydrationWarning={true}>
            <div className="w-full max-w-none mx-auto relative" suppressHydrationWarning={true}>
              {/* Resizable container with professional design */}
              <div
              ...
              >
                ...
                <NativeScriptEditor
                  ...
                  onContentChange={(content) => {
                    // ✅ CORRECTION: Receive content changes and update structure
        ...
        {/* Modern sidebar with statistics and navigation - Desktop only */}
        <div className="hidden lg:flex w-64 bg-gradient-to-b from-gray-800 to-gray-900 border-l border-gray-600/50 flex-col h-full shadow-xl" suppressHydrationWarning={true}>
          {/* Compact statistics with modern design */}
          <div className="p-2 border-b border-gray-700 flex-shrink-0 bg-gradient-to-r from-gray-800 to-gray-750" suppressHydrationWarning={true}>
            ...
            <div className="grid grid-cols-6 lg:grid-cols-3 gap-1 text-xs" suppressHydrationWarning={true}>
              <div className="bg-purple-600/20 border border-purple-500/30 p-1 rounded text-center hover:bg-purple-600/30 transition-colors touch-target" suppressHydrationWarning={true}>
                <div className="text-purple-300 text-xs font-medium" suppressHydrationWarning={true}>Chapters</div>
              ...
              <div className="bg-yellow-600/20 border border-yellow-500/30 p-1 rounded text-center hover:bg-yellow-600/30 transition-colors touch-target" suppressHydrationWarning={true}>
                <div className="text-yellow-300 text-xs font-medium" suppressHydrationWarning={true}>Panels</div>
              ...
              <div className="bg-green-600/20 border border-green-500/30 p-1 rounded text-center hover:bg-green-600/30 transition-colors touch-target" suppressHydrationWarning={true}>
                <div className="text-green-300 text-xs font-medium" suppressHydrationWarning={true}>Words</div>
              ...
              <div className="bg-gray-600/20 border border-gray-500/30 p-1 rounded text-center hover:bg-gray-600/30 transition-colors touch-target" suppressHydrationWarning={true}>
                <div className="text-gray-300 text-xs font-medium" suppressHydrationWarning={true}>Characters</div>
              ...
              {fileTree.map((page) => {
                ...
                {fileTree.length === 0 && (
                  <div className="text-gray-500 text-xs italic p-2" suppressHydrationWarning={true}>
                    [FR-UNTRANSLATED: Commencez à écrire pour voir la structure...]
                  </div>
                )}
              </div>
            </div>
