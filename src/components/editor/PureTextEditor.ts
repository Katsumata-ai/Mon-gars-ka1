...
export class PureTextEditor {
  ...
  private syntaxRules: SyntaxHighlightRule[] = [
    { regex: /^CHAPTER\s+\d+\s*:/gm, color: '#a855f7', fontWeight: '600' },
  ...
  private updateSyntaxHighlighting() {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      // Manual detection for better reliability
      if (/^CHAPTER\s+\d+\s*:/.test(trimmed)) {
        color = '#a855f7' // Violet
        fontWeight = '600'
        ...
        color = '#9ca3af' // Gray for descriptions
        fontWeight = '400'
      } else if (this.isDialogue(trimmed)) {
        color = '#3b82f6' // Blue for dialogues
        fontWeight = '500'
      }
  ...
  private isDialogue(line: string): boolean {
    // Dialogue with brackets: [CHARACTER] : or [CHARACTER]:
    if (/^\[.+\]\s*:/.test(line)) {
      return true
    }

    // Direct dialogue: HERO: or MARIE CLAIRE:
    // Starts with uppercase, can contain spaces and accents, ends with :
