...
export const useCanvasTransform = (
  ...
  const canvasScale = useMemo(() => {
    ...
    if (canvasTransformProp?.scale !== undefined) {
      console.log('🔧 useCanvasTransform: Utilisation du scale depuis canvasTransform:', canvasTransformProp.scale)
      return canvasTransformProp.scale
    }
    // Sinon, calculer depuis zoomLevel
    const calculatedScale = zoomLevel / 100
    console.log('🔧 useCanvasTransform: Calcul du scale depuis zoomLevel:', zoomLevel, '→', calculatedScale)
