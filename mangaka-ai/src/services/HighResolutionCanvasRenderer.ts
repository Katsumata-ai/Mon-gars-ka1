...
export class HighResolutionCanvasRenderer {
  ...
  private preparePageData(pageData: PageData): ExportPageData {
    ...
    // Debug : afficher la structure des éléments
    console.log('🔍 Debug - Éléments de la page:', elements.length)
    elements.forEach((el, index) => {
      console.log(`🔍 Élément ${index}:`, {
        type: el.type,
        id: el.id,
        transform: el.transform,
        hasImageUrl: !!(el as any).imageUrl,
        hasImageData: !!(el as any).imageData,
        hasText: !!(el as any).text,
        hasContent: !!(el as any).content,
        style: (el as any).panelStyle || (el as any).dialogueStyle || (el as any).textStyle
      })
    })
